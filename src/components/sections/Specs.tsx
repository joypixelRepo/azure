  ...
  return (
    ...
    >
      <div className="px-[var(--page-gutter)]">
        <div className="flex flex-wrap items-end justify-between gap-8">
          <Reveal>
            <div>
              ...
              <h2 className="display-lg mt-6 max-w-[14ch] text-strong">Los números, sin adornos.</h2>
            </div>
          </Reveal>
        ...
        <dl className="mt-20 grid grid-cols-1 border-t border-line sm:grid-cols-2 lg:grid-cols-3">
          {specs.map((spec, i) => (
            <Reveal key={spec.label} delay={Math.min(i * 55, 330)}>
              <div className="group flex h-full flex-col justify-between border-b border-line py-10 pr-6 transition-colors duration-700 hover:bg-gold/[0.04] sm:min-h-[12rem]">
                <dt className="spec-line eyebrow text-faint">{spec.label}</dt>
                <dd className="spec-line spec-line--value mt-6 flex items-center gap-4">
                  <span className="spec-icon">
                    <SpecIcon name={spec.icon} className="h-6 w-6" />
                  </span>
                  <span className="flex items-baseline gap-2">
                    <span className="display-lg text-[clamp(2.4rem,4.4vw,3.9rem)] leading-none text-strong">
          ...
          {specsFootnotes.map((row, i) => (
            <Reveal key={row.label} delay={Math.min(i * 55, 280)}>
              <div className="border-t border-line pt-5">
                ...
                <p className="body-sm mt-2 text-soft">{row.value}</p>
              </div>
            </Reveal>
          ))}
        </div>
